...
CONVERTED_SITES = [
    ...
    (STATE_IDLE, "if (Clock::openness() == Clock::Openness::Open && SystemConfig::get_connectionMode() == SystemConfig::CONNECTED) {", "dead-code never-auto-sleep check (Group B)"),
    (STATE_IDLE, "const bool openHoursKeepAwakeValid = (Clock::openness() == Clock::Openness::Open);", "idle-ceiling exemption (Group B)"),
    (STATE_SLEEP, "if (SystemConfig::get_connectionMode() == SystemConfig::CONNECTED && Clock::openness() != Clock::Openness::Closed) {", "sleep-abort-if-open (Group B)"),
