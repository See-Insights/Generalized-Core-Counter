...
// observedTimeSyncedLastMs()/reportedSyncAgeMs() now declared by
// time/Clock.h (WO-2026-09-18 Step 3a) - defined in time/Clock.cpp.
...
void setup() {
  ...
  // ACTUALLY compiled in, derived directly from the same #if conditions that
  // gate each feature (see src/BuildProfile.h) so it cannot silently drift
  // from the shipped binary the way ENABLE_BORON_USB_SOURCE_OVERRIDE did.
  ...
  Log.info("Boot: v=%s reset=%d heap=%lu prev=%s open=%d FailsafeTest=%d flags=0x%04x",
           FIRMWARE_VERSION,
           reason,
           ...
           (unsigned int)compiledBuildFlags);
