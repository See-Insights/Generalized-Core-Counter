...
# Release helper: bump version + notes, update Doxyfile + CHANGELOG, commit, and push.
#
# Usage:
...
usage() {
  cat <<'EOF'
Usage:
  ./release.sh [--yes] [--tag] [--dry-run] [--allow-dirty] <version> <release-notes...>
...
if [ "$ASSUME_YES" -ne 1 ]; then
  ...
  echo "  - update src/Version.cpp, Doxyfile, CHANGELOG.md"
...
# Step 1: bump version/notes + doc version + changelog entry
run ./bump_version.sh "$VERSION" "$RELEASE_NOTES"

# Step 2: stage known files
run git add src/Version.cpp Doxyfile CHANGELOG.md
